...
@app.get("/hboi")
async def hboi(
            architectuurlaag: str = Query(default=None, regex="Gebruikersinteractie|Organisatieprocessen|Infrastructuur|Software|Hardwareinterfacing"),
            activiteit: str = Query(default=None, regex="Analyseren|Adviseren|Ontwerpen|Realiseren|Manage & Control"), 
            niveau: str = Query(default=None, regex="1|2|3|4")
        ):
    hboi = json.load(open("json/hboi.json"))

    if architectuurlaag:
        hboi = {key: value for key, value in hboi.items() if architectuurlaag in key}
